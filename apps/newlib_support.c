...
// この関数を定義しないと、fopen を呼び出すことができない。
...
caddr_t sbrk(int incr) {
  ...
  static uint64_t program_break = 0; // static を付けているので、次の呼び出しでも値が保存されている。
  ...
  if (dpage_end == 0 || dpage_end < program_break + incr) {
    ...
    program_break = res.value; // res.value にはメモリを拡張する前の終端のアドレスが入っている。変数名がややこしい。
