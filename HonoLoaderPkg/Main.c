...
EFI_STATUS EFIAPI UefiMain (
  ...
  // カーネルファイルを一時的に読み出すためのメモリ領域を確保する。
  // 一時的に確保したメモリ領域にカーネルを読み出す。
  VOID* kernel_buffer;
  ...
  if (status == EFI_SUCCESS) {
    ...
    status = OpenBlockIoProtocolForLoadedImage(image_handle, &block_io); // block_io に起動メディアに関連付けられた Block I/O Protocol を取得する。
    if (EFI_ERROR(status)) {
  ...
  if (EFI_ERROR(status)) {
    ...
    status = gBS->ExitBootServices(image_handle, memmap.map_key);
    if (EFI_ERROR(status)) {
      Print(L"Could not exti boot service: %r\n", status);
      Halt();
    }
  }
  ...
  // FrameBuffer の Config オブジェクトを作成する
