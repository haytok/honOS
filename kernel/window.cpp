...
void Window::DrawTo(FrameBuffer& dst, Vector2D<int> pos, const Rectangle<int>& area) {
  ...
  if (!transparent_color_) {
    Rectangle<int> window_area{pos, Size()}; // ウィンドウの絶対座標とサイズを格納
    Rectangle<int> intersection = area & window_area;
    dst.Copy(intersection.pos, shadow_buffer_, {intersection.pos - pos, intersection.size}); // 第三引数には、intersection からの相対座標と重なる領域のサイズのオブジェクトを入れる。
    return;
  }
