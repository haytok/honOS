...
namespace {
  ...
  // CONFIG_ADDRESS レジスタ用の 32 bit 整数を生成する
  ...
  Error ScanBus(uint8_t bus) {
    // 1 つのバスに最大 32 個の PCI デバイスが接続されている。
  ...
  Error ConfigureMSIXRegister(const Device& dev, uint8_t cap_addrm,
                              uint32_t msg_addr, uint32_t msg_datam,
                              unsigned int num_vector_exponent) {
...
namespace pci {
  ...
  // ヘッダタイプは 8 ビット整数である。
  // また、ビット 7 が 1 の場合は、マルチファンクションデバイスである。
